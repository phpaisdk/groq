...
function configureCustomGroqWith(FakeHttpClient $client): void
{
...
it('uses a registered custom model for text generation', function () {
    ...
    configureCustomGroqWith($client);
    ...
    $result = Generate::text('Hi')->model(Groq::model('custom-groq-model'))->run();

    expect($result->text)->toBe('Hello from custom')
        ->and($result->usage->inputTokens)->toBe(7);

    $body = $client->sentBody();
    expect($body['model'])->toBe('custom-groq-model')
...
it('registers a custom model with the terse facade signature', function () {
    ...
    Groq::registerModel('custom-groq-model', capabilities: [
